...
const ReceiveRgbToken: React.FC = () => {
  ...
  // Render QR whenever we get a new invoice. Revoke the previous object URL
  // on change/unmount — createObjectURL pins the Blob in the URL registry for
  // the popup's lifetime otherwise, leaking one Blob per generated invoice.
  useEffect(() => {
    if (!result?.invoice) return;
    const url = qrGifDataUrl(result.invoice);
    setQrSrc(url);
