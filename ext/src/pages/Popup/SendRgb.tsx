...
const SendRgb: React.FC = () => {
  ...
  const broadcast = async () => {
    ...
    try {
      ...
      // A post-broadcast backup failure carries the txid (see
      // RgbWallet.criticalBackupAfterBroadcast). The transaction is ALREADY on
      // the network — returning to Prepared here would re-enable Confirm and a
      // second tap would broadcast a second payment.
      if (e?.txid) {
        setTxid(e.txid);
        setError('Sent, but cloud backup failed — it will retry on the next wallet change.');
        setStep(Step.Sent);
        return;
      }
  ...
  if (step === Step.Sent) {
    return (
      <div style={{ textAlign: 'center', padding: '20px' }}>
        ...
        {error ? <p style={{ color: '#ffb86b', fontSize: 12, marginBottom: 6 }}>{error}</p> : null}
