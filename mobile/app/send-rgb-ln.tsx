...
export default function SendRgbLnScreen() {
  ...
  // ONLY on the BOLT11 path. For an rgb:/utxob: invoice, lightningSendAsset
  // returns an on-chain transfer txid that never matches a paymentHash, so
  // polling there would spin forever without ever updating.
  useEffect(() => {
    if (!isBolt11) return;
  ...
  }, [result, network, accountNumber, isBolt11]);
