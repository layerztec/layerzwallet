...
/**
 * UTEXO's VSS (Versioned Storage Service) endpoint — cloud backup for the RLN
 * node's state. Passed as `vssUrl` in the node params: the node configures its
 * VSS client at `init()`, restores an existing backup when one exists, and
 * auto-backs-up on its own (plus our explicit `backupNow()` after mutations).
 * Signet URL provided by UTEXO 2026-09-09; mainnet pending like everything else.
 */
export const RGB_VSS_URL: Record<'signet' | 'mainnet', string | null> = {
  signet: 'https://vss-server.utexo.com/vss',
  mainnet: null,
};
