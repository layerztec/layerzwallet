...
import { Button, Dimensions, Modal, StyleSheet, TouchableOpacity, View, AppState, AppStateStatus, Alert } from 'react-native';
...
export const ScanQrContextProvider: React.FC<{ children: ReactNode }> = (props) => {
  ...
  const scanQr = async (): Promise<string> => {
    ...
    return new Promise(async (resolve) => {
      ...
      try {
        ...
        // Permissions are granted, show the modal scanner
        console.debug('ScanQr: Camera permission granted, showing scanner');
      } catch (error) {
    ...
    if (!permission.granted) {
      ...
      return (
        <View style={styles.container}>
          <TouchableOpacity style={[styles.closeButton, { top: insets.top + 10 }]} onPress={cancelCamera} testID="CloseCameraButton">
  ...
  return (
    <ScanQrContext.Provider value={{ scanQr, dismissScanner: cancelCamera, handleQrScanned }}>
      {props.children}
      <Modal visible={isModalVisible} animationType="slide" transparent={false} onRequestClose={cancelCamera}>
        <View style={styles.modalContainer}>{isScanningQr ? renderCameraFeed() : <View style={styles.container} />}</View>
      </Modal>
