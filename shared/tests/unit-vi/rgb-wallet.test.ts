...
describe('RgbWallet', () => {
  ...
  describe('backup-failure handling', () => {
    ...
    describe('tryBackup ledger', () => {
      ...
      it("does NOT throw on critical paths when VSS is unavailable — ledger records 'unavailable' and the banner takes over", async () => {
        // Mobile's shim throws a VssUnavailable-named error while UTEXO has no
        // VSS server. That's a permanent platform condition, not a failed
        // attempt: a successful transfer must not end in an error dialog, but
        // the ledger must still say "not backed up".
        const unavailable = Object.assign(new Error('VSS backup is not available: no VSS server URL is configured for this build'), { name: 'VssUnavailable' });
        const { sdkWallet } = installAdapter({
          vssBackup: vi.fn().mockRejectedValue(unavailable),
        });
        const storage = makeMemoryStorage();
        const w = new RgbWallet(NETWORK_RGB_TESTNET);
        w.setSecret(MNEMONIC);
        await w.init(storage);
        // issueAssetNia runs tryBackup({critical: true}) — must resolve.
        await expect(w.issueAssetNia({ ticker: 'X', name: 'X', precision: 0, amounts: [1] })).resolves.toBeDefined();
        expect(sdkWallet.issueAssetNia).toHaveBeenCalledOnce();
        const status = w.getBackupStatus();
        expect(status.pendingMutations).toBe(1);
        expect(status.lastBackupAt).toBeNull();
        expect(status.lastBackupError?.kind).toBe('unavailable');
      });
