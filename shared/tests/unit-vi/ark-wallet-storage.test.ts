 ...
 * every user's persisted Ark wallet state; a silent re-key orphans it and shows
 * up as a balance/history loss on update. If this test fails, you are re-keying
 * the namespace: ship a data migration for existing users, then update the
 * fixture consciously.
 *
...
test('storage namespaces are pinned for fixed inputs', async () => {
  ...
  // init() completed the first restore, so the bootstrap flag sits under the pinned namespace...
  assert.strictEqual(mutinynet.writes['ark-sdk-v2:1e777a4f929711a7:account_0:bootstrap:restoredV1'], 'true');
  ...
  assert.strictEqual(mainnet.writes['ark-sdk-v2:2e486c179cc55ef7:account_1:bootstrap:restoredV1'], 'true');
...
test('a list containing one corrupt row still yields the valid rows', () => {
  ...
  // BigInt(''), BigInt(null) and BigInt(false) are all 0n — such amounts must be rejected as corrupt, not kept as a zero balance
  const emptyAmountTx = { ...goodTxRaw, key: { ...goodTxRaw.key, arkTxid: 'f'.repeat(64) }, assets: [{ assetId: 'x', amount: '' }] };
  const nullAmountTx = { ...goodTxRaw, key: { ...goodTxRaw.key, arkTxid: '1'.repeat(64) }, assets: [{ assetId: 'x', amount: null }] };
  const falseAmountTx = { ...goodTxRaw, key: { ...goodTxRaw.key, arkTxid: '2'.repeat(64) }, assets: [{ assetId: 'x', amount: false }] };
  const txs = parseStoredTransactionList(JSON.stringify([null, { amount: 1 }, arrayKeyTx, badAssetTx, emptyAmountTx, nullAmountTx, falseAmountTx, goodTxRaw]));
  assert.strictEqual(txs.length, 1);
  assert.strictEqual(txs[0].key.arkTxid, 'd'.repeat(64));
});

test('contract repository is v2: watch round-trips and a missing watch reads as watched', async () => {
  const { storage } = makeCapturingStorage();
  const w = new ArkWallet();
  w.setSecret(TEST_MNEMONIC);
  await w.init(storage);

  const contractRepository = (Wallet.create as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0].storage.contractRepository;
  assert.strictEqual(contractRepository.version, 2);

  const base = { type: 'default', params: {}, address: 'ark1addr', state: 'active', createdAt: 1756199879000 };
  await contractRepository.saveContract({ ...base, script: 'aa' }); // legacy row: no watch field
  await contractRepository.saveContract({ ...base, script: 'bb', watch: 'retained' });

  assert.deepEqual(
    (await contractRepository.getContracts({ watch: 'watched' })).map((c: { script: string }) => c.script),
    ['aa']
  );
  assert.deepEqual(
    (await contractRepository.getContracts({ watch: 'retained' })).map((c: { script: string }) => c.script),
    ['bb']
  );
  assert.deepEqual(
    (await contractRepository.getContracts({ watch: ['watched', 'retained'] })).map((c: { script: string }) => c.script),
    ['aa', 'bb']
  );
  assert.strictEqual((await contractRepository.getContracts({ script: 'bb' }))[0].watch, 'retained');
});

test('concurrent contract saves do not lose each other (read-modify-write is serialized)', async () => {
  const { storage, writes } = makeCapturingStorage();
  // both saves read the blob before either writes it back, as they would under real async storage
  const slowStorage: IStorage = {
  ...
  const w = new ArkWallet();
  w.setSecret(TEST_MNEMONIC);
  await w.init(slowStorage);
  const contractRepository = (Wallet.create as ReturnType<typeof vi.fn>).mock.calls.at(-1)?.[0].storage.contractRepository;

  const base = { type: 'default', params: {}, address: 'ark1addr', state: 'active', createdAt: 1756199879000 };
  await Promise.all([contractRepository.saveContract({ ...base, script: 'aa' }), contractRepository.saveContract({ ...base, script: 'bb', watch: 'retained' })]);

  assert.deepEqual(
    (await contractRepository.getContracts()).map((c: { script: string }) => c.script),
    ['aa', 'bb']
  );
  assert.ok(Object.keys(writes).some((key) => key.endsWith(':contracts')));
});
