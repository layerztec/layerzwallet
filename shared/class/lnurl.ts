...
export interface LnurlPayServicePayload {
  callback: string;
  fixed: boolean;
  min: number;
  max: number;
...
export default class Lnurl {
  ...
  async requestBolt11FromLnurlPayService(amountSat: number, comment: string = ''): Promise<LnurlPayServiceBolt11Payload> {
    ...
    if (amountSat < this._lnurlPayServicePayload.min || amountSat > this._lnurlPayServicePayload.max)
      throw new Error('The specified amount is invalid, ' + amountSat + ' it should be between ' + this._lnurlPayServicePayload.min + ' and ' + this._lnurlPayServicePayload.max);
    ...
    const urlToFetch = this._lnurlPayServicePayload.callback + separator + 'amount=' + Math.floor(amountSat * 1000) + '&nonce=' + nonce + comment;
  ...
  getSuccessAction(): any | undefined {
    return this._lnurlPayServiceBolt11Payload && 'successAction' in this._lnurlPayServiceBolt11Payload ? this._lnurlPayServiceBolt11Payload.successAction : undefined;
  }
  ...
  getDisposable(): boolean | undefined {
    return this._lnurlPayServiceBolt11Payload && 'disposable' in this._lnurlPayServiceBolt11Payload ? this._lnurlPayServiceBolt11Payload.disposable : undefined;
  }
