...
export type RgbBackupErrorKind = 'network' | 'auth' | 'unavailable' | 'unknown';
/**
...
/**
 * Thrown (or matched by name/code) when the platform has no VSS backing store
 * at all — e.g. mobile before UTEXO publishes a VSS server URL. Unlike
 * 'network'/'auth' this is a permanent, known condition: the ledger records it
 * (so the warning banner shows "not backed up"), but `tryBackup` never
 * escalates it on critical paths — a successful transfer must not end in an
 * error dialog for a limitation the user cannot retry away.
 */
export const VSS_UNAVAILABLE = 'VssUnavailable';
...
export class RgbWallet extends AbstractWallet implements InterfaceAccountBasedWallet, InterfaceCanHaveTokens {
  ...
  async requestReceive(
    ...
    let out: { invoice: string; type: 'blind' | 'witness'; recipientId: string; expirationTimestamp: number | null };
    try {
      const r = await this.sdk().blindReceive(params);
      out = { invoice: r.invoice, type: 'blind', recipientId: r.recipientId, expirationTimestamp: r.expirationTimestamp };
    } catch (e) {
      ...
      out = { invoice: r.invoice, type: 'witness', recipientId: r.recipientId, expirationTimestamp: r.expirationTimestamp };
    }
    // The invoice created pending recipient state (a concealed seal) in the
    // node — without a backup, a recovered wallet can't recognize the
    // sender's transfer and the incoming assets are unreachable. Non-critical:
    // the invoice is still usable if the backup fails; the ledger records it.
    await this.tryBackup();
    return out;
  }
  ...
  async openLnChannel(request: Parameters<NonNullable<IRgbWallet['openChannel']>>[0]) {
    ...
    try {
      ...
      rgbLog('[rgb][openLnChannel] request:', JSON.stringify(request));
      rgbLog('[rgb][openLnChannel] unspents count:', unspents.length);
      for (const u of unspents) {
        rgbLog('[rgb][openLnChannel] utxo:', JSON.stringify(u));
      }
    } catch (probeErr: any) {
      rgbLog('[rgb][openLnChannel] pre-probe failed:', probeErr?.message ?? String(probeErr));
    }
    try {
      const r = await sdk.openChannel(request);
      rgbLog('[rgb][openLnChannel] response:', JSON.stringify(r));
      return r;
    } catch (e: any) {
      rgbLog('[rgb][openLnChannel] FAIL:', e?.name, e?.message, JSON.stringify(e));
      throw e;
    }
  }
  ...
  async pay(receiverAddress: string, amountSats: number): Promise<string> {
    ...
    // spendable. See tasks/ship-rgb.md. The tx is ALREADY broadcast at this
    // point, so a backup failure carries the txid — callers must treat it as
    // "sent, but not backed up", never re-enable a confirm button.
    await this.criticalBackupAfterBroadcast(txid);
    return txid;
  }

  /** tryBackup({critical:true}) for the post-broadcast paths: attaches the
   *  already-broadcast txid to the thrown error so UI catch-blocks can show
   *  "sent, but backup failed" instead of a retryable failure. */
  private async criticalBackupAfterBroadcast(txid: string): Promise<void> {
    try {
      await this.tryBackup({ critical: true });
    } catch (e: any) {
      try {
        ...
        // frozen error object — nothing to attach to
      }
      throw e;
    }
  }
  ...
  async transferToken(tokenId: string, amount: bigint, invoice: string, _memo?: string): Promise<string> {
    ...
    // tasks/ship-rgb.md. The transfer is already broadcast — the thrown
    // backup error carries the txid so callers show "sent, not backed up".
    await this.criticalBackupAfterBroadcast(result.txid);
    return result.txid;
  }
      ...
      try {
        ...
        if (sdk.listChannels) {
          try {
            ...
            rgbLog('[rgb][fetchTokenBalances] listChannels failed:', e?.message ?? e);
          }
        }
        ...
        if (sdk.getLspInfo && lnByAssetId.size > 0) {
          try {
            ...
            rgbLog('[rgb][fetchTokenBalances] getLspInfo failed:', e?.message ?? e);
          }
        }
  ...
  async getCommonTransactions(): Promise<CommonTransaction[]> {
    ...
    for (const t of transfers) {
      ...
      rgbLog('[rgb][getCommonTransactions] transfer:', JSON.stringify({ idx: t.idx, kind: t.kind, status: t.status, createdAt: t.createdAt, updatedAt: t.updatedAt, resolvedTimestamp: timestamp }));
      common.push({
        network: this._network,
        txid: t.txid ?? key,
        timestamp,
    ...
    if (sdk.listPaymentsRaw) {
      try {
        ...
        rgbLog('[rgb][getCommonTransactions] LN payments count:', payments.length);
        for (const p of payments) {
          rgbLog('[rgb][getCommonTransactions] LN payment:', JSON.stringify(p));
        ...
        rgbLog('[rgb][getCommonTransactions] listPaymentsRaw failed:', e?.message ?? e);
      }
    } else {
      rgbLog('[rgb][getCommonTransactions] SDK has no listPaymentsRaw — LN history omitted');
    }
  ...
  private async tryBackup(opts: { critical?: boolean } = {}): Promise<boolean> {
    ...
    try {
      ...
      // 'unavailable' is a permanent platform condition, not a failed attempt —
      // the ledger already flags it for the banner; escalating would turn every
      // successful transfer into an error dialog the user can't act on.
      if (opts.critical && kind !== 'unavailable') throw e;
      return false;
    }
  }
...
function classifyBackupError(e: unknown): RgbBackupErrorKind {
  ...
  if (err.code === VSS_UNAVAILABLE || err.name === VSS_UNAVAILABLE) return 'unavailable';
