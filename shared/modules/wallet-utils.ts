...
export async function lazyInitWallet(network: TSupportedLazyInitWalletNetworks, accountNumber: number, storage: IStorage, secureStorage: IStorage): Promise<TLazyInitedWallets> {
  ...
  if (network === NETWORK_RGB || network === NETWORK_RGB_TESTNET) {
    // The RLN node takes the whole mnemonic (`rlnInitNode(password, mnemonic)`)
    // and derives its keys internally — there is no account parameter anywhere
    // in rgb-sdk-rn (create params, unlock, signer). Every accountNumber would
    // hit the same node and the same funds, while per-account cache/lock keys
    // and the backup ledger's per-account storage key would split one wallet's
    // state across several keys. Pin to 0 (same policy as Liquid above) until
    // the SDK grows real account support — or we derive per-account child
    // seeds, which means one RLN node + one LSP channel per account.
    accountNumber = 0;
  }
