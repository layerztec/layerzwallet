...
export const HomeBalance: React.FC = () => {
  ...
  const [displayBalance, displaySubBalance] = useMemo(() => {
    ...
    return [formattedBalance, formatFiatBalance(balance, decimals, exchangeRate)];
    ...
    <div className="home-balance" data-testid="LayerBalance">
      {getIsTestnet(network) ? (
        <div className="home-testnet-warning">
          <ThemedText style={{ fontSize: 12, color: '#F59E0B' }}>Warning: You are using a testnet, coins have no value</ThemedText>
        </div>
      ) : null}
      ...
      <div className="home-balance-fiat">{displaySubBalance !== '—' ? `${displaySubBalance} USD` : '—'}</div>
    </div>
  );
};
