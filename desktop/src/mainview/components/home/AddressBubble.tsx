...
export const AddressBubble: React.FC<{
  ...
  const formatAddress = (addr: string) => {
    ...
    const middlePart1 =
      addr.length < 43
        ? middlePart.slice(0, splitIndex)
        : middlePart.slice(0, 16) + "...";
    const middlePart2 =
      addr.length < 43
        ? middlePart.slice(splitIndex)
        : "..." + middlePart.slice(middlePart.length - 16);

    return (
      <>
        <div className="wallet-address-bubble-line">
          <span className="wallet-address-bubble-strong wallet-address-bubble-strong--start">
            {firstPart}
          </span>
        ...
        <div className="wallet-address-bubble-line">
          ...
          <span className="wallet-address-bubble-strong wallet-address-bubble-strong--end">
            {lastPart}
          </span>
        </div>
      </>
  ...
  return (
    <div className="wallet-address-row" style={{ position: "relative" }}>
      ...
      {copied ? (
        <div className="wallet-address-copied-tooltip">Copied!</div>
      ) : null}
    </div>
  );
};
