...
export const TokenRow = memo(({ token, onPress, selected, onVisible }: TokenRowProps) => {
  ...
  const fiatLabel = balance && tokenExchangeRate && tokenExchangeRate > 0 ? '$' + formatFiatBalance(balance, token.decimals, tokenExchangeRate) : null;
  ...
  return (
    <RowTag
      ...
      className={['home-token-row', onPress ? 'home-token-row--clickable' : '', selected ? 'home-token-row--selected' : ''].filter(Boolean).join(' ')}
      onClick={onPress ? () => onPress(token) : undefined}
      data-testid={`token-row-${token.id}`}
    >
      <div className="home-token-icon" style={{ backgroundColor: iconColor }}>
        {localIcon ? <img src={localIcon} alt="" /> : token.logoURI ? <img src={token.logoURI} alt="" /> : <span className="home-token-icon-fallback">{token?.symbol?.charAt(0) || '?'}</span>}
      </div>
      <span className="home-token-name">{token?.name}</span>
      <div className="home-token-amounts">
        <span className="home-token-amount" data-testid={`token-amount-${token.id}`}>
