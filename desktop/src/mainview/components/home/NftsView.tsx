...
const NftPreviewItem: React.FC<{
  ...
  return (
    <button
      type="button"
      className={[
        "home-nft-preview-item",
        selected ? "home-nft-preview-item--selected" : "",
      ]
        .filter(Boolean)
        .join(" ")}
      onClick={() => onPress(nft)}
    ...
    >
      <div
        className="home-nft-preview-image-wrap"
        style={{ backgroundColor: iconColor }}
      >
        {nft.image ? (
          <NftImage uri={nft.image} alt="" className="home-nft-preview-image" />
        ) : (
          <span className="home-nft-preview-fallback">
            {nft?.name?.charAt(0) || "?"}
          </span>
        )}
      </div>
...
export const NftsView = forwardRef<{ refresh: () => void }, NftsViewProps>(
  ({ selectedNft, onViewGalleryPress }, ref) => {
    const navigate = useNavigate();
    const { network } = useContext(NetworkContext);
    const { accountNumber } = useContext(AccountNumberContext);
    const { nftList, error, mutate } = useNftDiscovery(
      network,
      accountNumber,
      BackgroundCaller,
      LayerzStorage,
      ...
      <div className={hide ? "home-nfts-hidden" : undefined}>
        <SectionContainer title="NFTs">
          ...
          {hasMoreThanPreview ? (
            <button
              type="button"
              className="home-nfts-view-gallery"
