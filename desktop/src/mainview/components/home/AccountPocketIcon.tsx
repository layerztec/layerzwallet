...
export const AccountPocketIcon: React.FC<AccountPocketIconProps> = ({
  item,
  variant = "header",
}) => {
  ...
  return (
    <span
      className={[
        "account-pocket-icon",
        variant === "menu" ? "account-pocket-icon--menu" : "",
      ]
        .filter(Boolean)
        .join(" ")}
      aria-hidden
    >
      <Icon
        size={variant === "menu" ? 24 : 22}
        color="#ffffff"
        strokeWidth={2}
        className={
          isMaterial ? "account-pocket-icon__glyph--material" : undefined
        }
      />
    </span>
  );
...
export const AccountPocketOptionLabel: React.FC<{ item: AccountItem }> = ({
  item,
}) => (
