...
export const MnemonicWordGrid: React.FC<MnemonicWordGridProps> = ({ words, loading }) => {
    ...
    <div className="onboarding-mnemonic-grid">
      {slots.map((word, index) => (
        <div key={index} className="onboarding-word">
          ...
          {loading ? <span className="onboarding-word__spinner" /> : <span className="onboarding-word__text">{word}</span>}
        </div>
      ))}
    </div>
  );
