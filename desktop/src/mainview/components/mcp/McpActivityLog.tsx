...
import { getMcpActivityLog, mcpActivityLogIsFull, subscribeMcpActivityLog } from '../../features/mcp/mcp-activity-log-desktop';
...
export const McpActivityLog: React.FC = () => {
  ...
  return (
    <div className="mcp-activity-log" aria-live="polite" aria-label="MCP activity log">
      {activityLines.map((line, index) => {
        const last = index === activityLines.length - 1;
        const oldestFading = activityLogFull && index === 0;
        const lineClass = last ? 'mcp-activity-line--latest' : oldestFading ? 'mcp-activity-line--fading' : 'mcp-activity-line--older';
        return (
          <p key={`${index}-${line}`} className={`mcp-activity-line ${lineClass}`}>
            {line}
          </p>
        );
      })}
