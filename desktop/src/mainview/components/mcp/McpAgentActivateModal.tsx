...
export const McpAgentActivateModal: React.FC<McpAgentActivateModalProps> = ({
  onClose,
}) => {
  ...
  return (
    <DetachedSheet variant={network} onClose={onClose}>
      <div className="mcp-agent-activate-body">
        <button
          type="button"
          className="mcp-agent-activate-close"
          onClick={onClose}
          aria-label="Close"
        >
          <X size={18} />
        ...
        <Bot
          className="mcp-agent-activate-icon"
          size={52}
          strokeWidth={1.5}
          aria-hidden
        />
        <h2 className="mcp-agent-activate-title">Agent</h2>
        <p className="mcp-agent-activate-subtitle">
          Automate payments, trade and control your wallet from your messaging
          or AI provider.
        </p>
        <button
          type="button"
          className="mcp-agent-activate-btn"
          onClick={handleActivate}
          aria-label="Activate agent"
          data-testid="McpAgentActivateButton"
        >
          Activate
        </button>
      </div>
