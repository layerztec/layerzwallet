...
const BalanceDefault = forwardRef<{ refresh: () => void }, BalanceProps>(
  ({ network, accountNumber, BackgroundCaller }, ref) => {
    ...
    const handleBuyClick = () => {
      BackgroundCaller.getAddress(network, accountNumber).then(
        (addressResponse: string) => {
          if (addressResponse) {
            window.open(
              `https://layerztec.github.io/website/onramp/?address=${addressResponse}&network=${network}`,
              "_blank",
            );
          }
        },
    ...
    return (
      <>
        {getIsTestnet(network) ? (
          <div
            style={{ color: "darkred", width: "100%", marginBottom: "15px" }}
          >
        ...
        <h1>
          ...
          <span style={{ fontSize: 14 }}>
            {displaySubBalance !== "—" ? `$${displaySubBalance}` : ""}
          </span>
          {canBuyWithFiat ? (
        ...
        <span
          id="pocket-balance"
          style={{ position: "absolute", top: 0, left: 20, fontSize: 14 }}
        >
          Pocket balance:{" "}
          {accountBalance
            ? formatBalance(
                accountBalance,
                getDecimalsByNetwork(NETWORK_BITCOIN),
                8,
              )
            : ""}{" "}
...
const BalanceLightning = forwardRef<{ refresh: () => void }, BalanceProps>(
  ({ network, accountNumber, BackgroundCaller }, ref) => {
    const availableNetworks = useAvailableNetworks();
    const { accountBalance } = useAccountBalance(
      accountNumber,
    ...
    // Lightning network aggregates balances from Spark and Liquid
    const liquidNetwork =
      network === NETWORK_LIGHTNING_TESTNET
        ? NETWORK_LIQUID_TESTNET
        : NETWORK_LIQUID;
    const { balance: sparkBalance, mutate: mutateSpark } = useBalance(
      NETWORK_SPARK,
      accountNumber,
      BackgroundCaller,
    );
    const { balance: liquidBalance, mutate: mutateLiquid } = useBalance(
      liquidNetwork,
      accountNumber,
      BackgroundCaller,
    );
    ...
    const { exchangeRate: sparkExchangeRate } = useExchangeRate(
      NETWORK_SPARK,
      "USD",
    );
    const { exchangeRate: liquidExchangeRate } = useExchangeRate(
      liquidNetwork,
      "USD",
    );
    ...
    >(() => {
      if (network === NETWORK_LIGHTNING) {
        ...
        const totalUsdValue = BN(liquidBalance)
          .times(liquidExchangeRate)
          .dividedBy(BN(10).pow(decimals))
          .toFixed(2);
        return [formattedBalance, totalUsdValue];
      }
    ...
    const rows = useMemo(() => {
      const networks =
        network === NETWORK_LIGHTNING
          ? [NETWORK_SPARK, NETWORK_LIQUID]
          : [NETWORK_LIQUID_TESTNET];

      return networks.map((net, index) => {
        let balance: string | undefined;
        let exchangeRate: number | undefined;
        if (net === NETWORK_SPARK) {
        ...
        const formattedBalance =
          balance !== undefined
            ? formatBalance(balance, Number(getDecimalsByNetwork(net)), 8)
            : "—";
        const formattedFiatBalance =
          exchangeRate !== undefined && balance !== undefined
            ? "$" +
              formatFiatBalance(
                balance,
                Number(getDecimalsByNetwork(net)),
                Number(exchangeRate),
              )
            : "$—";
        ...
        return (
          <tr
            ...
            style={{
              borderBottom:
                index < networks.length - 1 ? "1px solid #eee" : "none",
            }}
          >
            <td style={{ padding: "8px", fontSize: "14px" }}>{networkName}</td>
            <td
              style={{ textAlign: "right", padding: "8px", fontSize: "14px" }}
            >
...
const BalanceUsdt = forwardRef<{ refresh: () => void }, BalanceProps>(
  ({ network, accountNumber, BackgroundCaller }, ref) => {
    const availableNetworks = useAvailableNetworks();
    const { accountBalance } = useAccountBalance(
      accountNumber,
    ...
    const { tokenList: rsTokenListOrig, mutate: mutateRsTokens } =
      useTokenDiscovery(
        NETWORK_ROOTSTOCK,
        accountNumber,
        BackgroundCaller,
    ...
    const { tokenList: liquidTokenListOrig, mutate: mutateLiquidTokens } =
      useTokenDiscovery(
        NETWORK_LIQUID,
        accountNumber,
        BackgroundCaller,
...
const Balance = forwardRef<{ refresh: () => void }, BalanceProps>(
  ({ network, accountNumber, BackgroundCaller }, ref) => {
    if (
      network === NETWORK_LIGHTNING ||
      network === NETWORK_LIGHTNING_TESTNET
