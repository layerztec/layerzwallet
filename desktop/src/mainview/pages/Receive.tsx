...
const Receive: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      const wallet = await BackgroundCaller.lazyInitWallet(
        network,
        accountNumber,
      );
      if (cancelled) return;
      ...
      const poll = async () => {
        ...
        for (const token of currentTokens) {
          ...
          const initial = new BigNumber(
            tokenInitialRef.current?.get(key) ?? "0",
          );
  ...
  if (
    (network === NETWORK_STACKS || network === NETWORK_SPARK) &&
    stacksTokenReceiveInfo
  ) {
    return wrapPage(
      <div style={{ position: "relative" }}>
        <ThemedText type="headline">
          Receive on {network.charAt(0).toUpperCase() + network.slice(1)}
        </ThemedText>

        <div style={{ textAlign: "center", padding: "20px" }}>
          <div
            style={{ color: "#4CAF50", fontSize: "48px", marginBottom: "20px" }}
          >
            ✓
          </div>
          <h2 style={{ color: "#4CAF50", marginBottom: "15px" }}>
            <ThemedText type="headline">
              Received: +
              {formatBalance(
                String(stacksTokenReceiveInfo.amountDelta),
                stacksTokenReceiveInfo.decimals,
                8,
              )}{" "}
              {stacksTokenReceiveInfo.symbol}
            </ThemedText>
          </h2>
          <div style={{ color: "#666", fontSize: "14px", marginBottom: "4px" }}>
            {stacksTokenReceiveInfo.name}
          </div>
  ...
  return wrapPage(
    <div style={{ position: "relative" }}>
      ...
      <WalletToolButton
        ...
        data-testid="receive-back-button"
      >
        Back to Wallet
      </WalletToolButton>
    </div>,
