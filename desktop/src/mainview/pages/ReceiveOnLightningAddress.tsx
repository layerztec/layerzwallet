...
const ReceiveOnLightningAddress: React.FC = () => {
  ...
  const isNewBalanceGT = useCallback((): false | StringNumber => {
    if (
      Boolean(balance && oldBalance && new BigNumber(balance).gt(oldBalance))
    ) {
  ...
  const fetchAddress = useCallback(async () => {
    setIsLoading(true);
    try {
      const addressResponse = await BackgroundCaller.getAddress(
        network,
        accountNumber,
      );
  ...
  const handleReceiveOnLiquid = () => {
    const n =
      networkFromContext === NETWORK_LIGHTNING_TESTNET
        ? NETWORK_LIQUID_TESTNET
        : NETWORK_LIQUID;
  ...
  const lightningReceiveActions = [
    {
      children: (
        <ActionPopupAction
          testID="ReceiveOnSparkButton"
          network={NETWORK_SPARK}
          text="Receive on Spark"
        />
      ),
    ...
    {
      children: (
        <ActionPopupAction network={NETWORK_LIQUID} text="Receive on Liquid" />
      ),
        ...
        {received ? (
          <div className="receive-lightning-success">
            ...
            <ThemedText type="headline" data-testid="NetworkAddressHeader">
              ...
              {formatBalance(
                String(received),
                getDecimalsByNetwork(network),
                getDecimalsByNetwork(network),
              )}{" "}
              {getTickerByNetwork(network)}
            </ThemedText>
            <br />
            <WideButton onClick={() => navigate("/home")}>
              Back to Wallet
            </WideButton>
          </div>
        ) : (
          <>
            ...
            <div className="receive-lightning-qr-wrap">
              {!isLoading && lightningAddress ? (
                <>
                  <div
                    className="receive-lightning-qr"
                    data-testid="LightningAddressQrContainer"
                  >
                    {imgSrc ? (
                      <img
                        src={imgSrc}
                        alt="Lightning address QR code"
                        data-testid="LightningAddressQrCode"
                      />
                    ) : null}
                  </div>
                  <div
                    className="receive-lightning-address"
                    data-testid="LightningAddressButton"
                  >
                    <span>
                      {(resolvedUsername || lightningAddressParts?.local) ?? ""}
                    </span>
                    {(resolvedUsername || lightningAddressParts?.domain) && (
                      <span className="receive-lightning-domain">
                        @
                        {resolvedUsername
                          ? "layerz.me"
                          : lightningAddressParts?.domain}
                      </span>
                    )}
                  </div>
                </>
                ...
                <div
                  className="receive-lightning-qr receive-lightning-qr-placeholder"
                  data-testid="QrContainer"
                >
                  {isLoading ? (
                    <>
                      <span data-testid="LoadingPlaceholder">
                        Loading address...
                      </span>
                    </>
                  ) : (
            ...
            <div className="receive-lightning-actions">
              <button
                type="button"
                className="receive-lightning-btn"
                data-testid="ShareButton"
                onClick={() => void handleShare()}
                disabled={!lightningAddress || isSharing}
              >
                Share...
              </button>
              <ActionPopupButton
                actions={lightningReceiveActions}
                title="Layer to receive on"
              >
                <button
                  type="button"
                  className="receive-lightning-btn"
                  data-testid="ReceiveOnLightningAddressWithAmountButton"
                >
                  Receive with amount
                </button>
              </ActionPopupButton>
